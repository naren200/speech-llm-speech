...
function launch_container_terminal {
    ...
    if [ "$DEVELOPER" = "True" ]; then
        gnome-terminal --title="DEV_TERM_$name" -- bash -c "docker exec -it $container_id /bin/bash || read -p 'Press Enter to close...'"
        echo "DEV_TERM_$name" >> "$TERMINAL_IDS_FILE"
    else
        if [ ! -z "$pkg" ]; then
            gnome-terminal --title="RUN_TERM_$name" -- bash -c "docker exec -it $container_id /bin/bash -l -c \"/bin/bash /root/ros2_ws/src/speech-llm-speech${pkg}/start_in_docker.sh\" || read -p 'Press Enter to close...'"
            echo "RUN_TERM_$name" >> "$TERMINAL_IDS_FILE"
        fi
    fi
}
